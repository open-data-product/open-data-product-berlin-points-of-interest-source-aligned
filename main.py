...
def main(clean, quiet):
    ...
    extract_overpass_data(
        ...
        bounding_box_geojson_path=os.path.join(
            bronze_path, "berlin-lor-city", "berlin-lor-city.geojson"
        ),
    ...
    results_json_path = os.path.join(
        gold_path,
        f"berlin-points-of-interest-{datetime.now().strftime("%Y")}-{datetime.now().strftime("%m")}-json",
    )
    os.makedirs(results_json_path, exist_ok=True)
    shutil.copy(
        os.path.join(
            bronze_path,
            f"berlin-points-of-interest-{current_year}-{current_month}",
            "berlin-points-of-interest-forests-details.json",
        ),
        results_json_path,
    )
    shutil.copy(
        os.path.join(
            bronze_path,
            f"berlin-points-of-interest-{current_year}-{current_month}",
            "berlin-points-of-interest-parks-details.json",
        ),
        results_json_path,
    )
